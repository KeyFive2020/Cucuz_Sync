...
import br.com.cuscuz.sync.client.screen.SyncReportSavedScreen;
...
public final class ClientSyncController {
    ...
    public static void prepare(ConnectionRequest request, SyncManifest manifest, SyncPlan plan,
        ...
        CompletableFuture.supplyAsync(() -> {
            try {
                return SyncInstaller.prepare(
                        minecraft.gameDirectory.toPath(), manifest, plan, exportMissingReport);
            ...
            minecraft.setScreen(new SyncPreparedScreen(
                    request.parent(), prepared.operationCount(), prepared.blockedCount(), prepared.blockedReport()));
        }));
    }

    public static void exportMissingReport(ConnectionRequest request, Screen returnScreen,
                                           SyncManifest manifest, SyncPlan plan) {
        Minecraft minecraft = Minecraft.getInstance();
        if (!PREPARING.compareAndSet(false, true)) {
            return;
        }
        minecraft.setScreen(new SyncProgressScreen(Component.literal("Criando lista de mods sem fonte...")));
        CompletableFuture.supplyAsync(() -> {
            try {
                return SyncInstaller.exportBlockedReport(minecraft.gameDirectory.toPath(), manifest, plan);
            } catch (Exception exception) {
                throw new CompletionException(exception);
            }
            ...
            minecraft.setScreen(new SyncReportSavedScreen(returnScreen, report));
        }));
    }
